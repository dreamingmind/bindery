...
//            $this->redirect($this->referer());
        ...
        if (!empty($this->request->data)) {
//            debug($this->request->data['ContentCollection'][0]);

            $result = 0; // 0 at the end means nothing saved properly
            $result_message = '';
            //save the Content portion
            if ($this->Content->save($this->request->data)) {
                $result_message = __('The content has been saved');
                $result += 1;
            } else {
                $result_message = __('The content could not be saved. Please, try again.');
            }
            if (isset($this->request->data['ContentCollection'][0]['Supplement'])) {
                if ($this->Content->ContentCollection->Supplement->saveAll($this->request->data['ContentCollection'][0]['Supplement'])) {
                    $result_message .= __('<br />The supplements have been saved');
                    $result += 1;
                } else {
                    $result_message .= __('<br />The supplements could not be saved. Please, try again.');
                }
            }
            if ($this->Content->Image->save($this->request->data)) {
                $result_message .= __("<br />The image has been saved");
                $result += 1;
            } else {
                $result_message .= __("<br />The image could not be saved. Please, try again.");
            }
            $this->Session->setFlash($result_message);
            if ($result > 0) {
                // refresh the screen after everythign is saved
                $this->passedArgs = unserialize($this->request->data['Content']['passedArgs']);
                $this->request->params = unserialize($this->request->data['Content']['params']);
                $this->gallery();
                $this->render('gallery', 'thumbnailPage');
            } else {
                // SEE ISSUE 82 FOR THE DIRECTION TO GO ON THIS SECTION
                // if nothing saved, redraw the form
                $packet = $this->request->data;
...
//                $inlineImages[$index]['image'] = $image['Content']['Image']['img_file'];
    ...
    function sequence() {
        ...
        if (isset($this->request->data[0])) {
            //save the data here
            $this->Content->ContentCollection->saveAll($this->request->data);
            $this->blogPage();
...
//            ,'Content.publish'=>1
            ...
            } elseif (is_numeric($this->request->params['pass'][0])) {
                $conditions['ContentCollection.collection_id'] = $this->request->params['pass'][0];

                if (isset($this->request->params['pass'][1]) && is_string($this->request->params['pass'][1])) {
                    $conditions['Content.slug'] = $this->request->params['pass'][1];
                }
            } elseif (is_string($this->request->params['pass'][0])) {
                $conditions['Content.slug'] = $this->request->params['pass'][0];
                $most_recent = $this->readMostRecentBlog($conditions);
                $conditions['ContentCollection.collection_id'] = $most_recent['Collection']['id'];
            }
//        debug($most_recent);
...
//                'conditions'=>array(
//                    'Navline.route'=>  $this->request->params['pname']
//                )
//            ));
...
//        }
        ...
        $this->set('collection', $this->Content->ContentCollection->Collection->find('first', array(
                    'conditions' => array(
                        ...
                        'Collection.slug' => $this->request->params['pname']
                    ),
                    'recursive' => -1
                )));
...
//        $this->set('result_imagePath',  $this->result_imagePath);
        ...
        $this->set('recentNews', $this->Content->recentNews(2, $this->request->params['pname']));
        $this->set('recentExhibit', $this->Content->recentExhibits(2, $this->request->params['pname']));
        $sale_items = $this->Catalog->find('all', array(
            'conditions' => array('category' => $this->request->params['pname'])
                ));
     ...
     * @param string $pname The product group (normally comes in on $this->request->params['pname'])
     */
...
//        debug($this->passedArgs);
        ...
        $pname = (isset($this->request->params['pname'])) ? $this->request->params['pname'] : null;
...
//        $pname = (isset($this->request->params['pname'])) ? $this->request->params['pname'] : null;
    ...
    function jumpGallery() {
        ...
        $this->pageConditions = array(
            'Collection.slug' => $this->request->data['pname'],
            'Collection.category_id' => $this->categoryNI['exhibit']//$this->category[0]['Category']['id']
        );
        $neighbors = $this->filmstripNeighbors();
        $target = array_slice($neighbors, $this->request->data['j'] - 1, 1, TRUE);
        ...
        if (preg_match($patterns[0], $this->request->data['url'])) {
            return preg_replace($patterns, $replacements, $this->request->data['url']);
        } else {
            return $this->request->data['url'] . '/page:' . $target[$key[0]]['page'] . '/' . 'id:' . $key[0];
        }
    }
...
//            'fields' => $this->pageFields,
     ...
     */
    function search() {
        if ($this->verifySearchData($this->request->data)) {
            ...
            if ($this->request->params['data']['action'] == 'blog') {
                $this->readBlogTOC();
                $this->layout = 'blog_layout';
            } else {
                $this->layout = 'noThumbnailPage';
