...
class NavigatorsController extends AppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid Navigator.'));
	...
	function add() {
		if (!empty($this->request->data)) {
			...
			if ($this->Navigator->save($this->request->data)) {
				$this->Session->setFlash(__('The Navigator has been saved'));
				$this->redirect(array('action'=>'index'));
			} else {
				$this->Session->setFlash(__('The Navigator could not be saved. Please, try again.'));
			}
		}
	...
	function edit($id = null) {
		...
		if (empty($this->request->data)) {
			$this->request->data = $this->Navigator->read(null, $id);
		}
		$navlines = $this->Navigator->Navline->find('list');
		$this->set(compact('navlines'));
	}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for Navigator'));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->Navigator->delete($id)) {
			$this->Session->setFlash(__('Navigator deleted'));
			$this->redirect(array('action'=>'index'));
		}
	}
