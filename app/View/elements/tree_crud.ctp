...
<?php
	$row = 1;
 	foreach($this->request->data['treeCrudList']['tree'] as $key => $treeLine) {
...
$(document).ready(function() {
 ...
 function showme() {
    $('div#liveFunction > fieldset').prependTo('div#functionPool').hide(200);
    $('div#liveFunction').html($('fieldset[id ~= \''+$('input:radio[name="data[{$this->request->data['primaryModel']}][action]"]:checked').val()+'\']').show(200));
 }
JSC;
        $this->Js->buffer($code);
        $this->Js->buffer($this->Js->get("fieldset[class*='function']")->each('$(this).hide();'));
        $this->Js->buffer('showme();'); //make sure any selected radio button reveals its form on page refresh

        $params = '';
        if (isset($this->request->params['named']['lft'])&& isset($this->request->params['named']['rght'])) {
            $params = '/lft:'.$this->request->params['named']['lft'].'/rght:'.$this->request->params['named']['rght'];
        }
