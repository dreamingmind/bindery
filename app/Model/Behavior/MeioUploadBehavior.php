 ...
 * @access public
 */
	function uploadCheckFieldName(Model $model, $data) {
		foreach ($data as $fieldName => $field) {
			...
			if (isset($this->__fields[$model->alias][$fieldName])) {
				...
				trigger_error(sprintf(__d('meio_upload', 'MeioUploadBehavior Error: The field "%s" wasn\'t declared as part of the MeioUploadBehavior in model "%s".'), $fieldName, $model->alias), E_USER_WARNING);
				return false;
			}
		}
		return true;
 ...
 * @access public
 */
	function uploadCheckMinWidth(Model $model, $data) {
 ...
 * @access public
 */
	function uploadCheckMinHeight(Model $model, $data) {
 ...
 * @access public
 */
	function uploadCheckMaxHeight(Model $model, $data) {
 ...
 * @access protected
 */
	function _uploadCheckSize(Model $model, &$data, $type) {
		foreach ($data as $fieldName => $field) {
 ...
 * @access protected
 */
	function _createThumbnails(Model $model, $fieldName, $saveAs, $ext, $options) {
 ...
 */
	function _createThumbnail(Model $model, $source, $target, $fieldName, $params = array()) {
		...
		if ($w && $h) {
			...
			trigger_error(__d('meio_upload', 'Width and Height of thumbs not specified.'), E_USER_WARNING);
			return;
		}
		...
		if ($phpThumb->GenerateThumbnail()) {
			if (!$phpThumb->RenderToFile($target)) {
				trigger_error(sprintf(__d('meio_upload', 'Could not render image to: %s'), $target), E_USER_WARNING);
			}
		}
	}
 ...
 * @access protected
 */
	function _replaceTokens(Model $model, $string, $fieldName) {
		return str_replace(
 ...
 * @access protected
 */
	function _fixName(Model $model, $fieldName, $checkFile = true) {
 ...
 */
	function _sizeToBytes($size) {
		...
		if (!preg_match('/^(\d+) ?(kb|mb|gb|tb)$/i', $size, $matches)) {
			trigger_error(__d('meio_upload', 'MeioUploadBehavior Error: The maxSize option format is invalid. Using 2 MB (default).'), E_USER_WARNING);
			return 2097152;
		}
 ...
 * @access protected
 */
	function _setupValidation(Model $model, $fieldName, $options) {
		$options = $this->__fields[$model->alias][$fieldName];
 ...
 */
	function _copyFileFromTemp($tmpName, $saveAs) {
		...
		if (!move_uploaded_file($tmpName, $saveAs)) {
			return __d('meio_upload', 'Problems in the copy of the file.');
		}
		return true;
	}
 ...
 * @access protected
 */
	function _setFileToRemove(Model $model, $fieldName) {
 ...
 * @access protected
 */
	function _cleanFields(Model $model, $fieldName) {
