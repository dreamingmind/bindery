...
//    debug($most_recent);die;
    echo $this->Form->create('Content', array('default'=>false));
        ...
        echo '<fieldset>';
        echo $form->input("ContentCollection.$count.id",array(
            'type'=>'hidden',
            'value'=>$entry['ContentCollection']['id'],
            'name'=>"data[$count][ContentCollection][id]"
        ));
        echo $form->input("ContentCollection.$count.seq",array(
            'value'=>$entry['ContentCollection']['seq'],
            'name'=>"data[$count][ContentCollection][seq]"
            ));
        echo $form->input("ContentCollection.$count.publish",array(
            'type'=>'text',
        ...
        echo $html->para(null,$text->truncate($entry['Content']['content'],200,array('ending'=>'...')));
//        echo $entry['Content']['content'];
       echo '</fieldset>';
        }
    echo $this->Form->end(array('label'=>__('Submit', true),'id'=>'sequence'));    
//    debug($seq);
    ?>
    <div id="result"></div>
</div>
...
//        alert('<?php //echo $this->params['url']['url'] ?>');
        $.post('<?php echo $this->params['url']['url'] ?>',$('#ContentSequenceForm').serialize(),function(data){
            $('#detail').html(data);
        });
    })
