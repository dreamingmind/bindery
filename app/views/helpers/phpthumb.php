...
class PhpthumbHelper extends Helper    {
    ...
    private function create_thumb()    {
        ...
        if($this->php_thumb->GenerateThumbnail()) {
            debug ($this->php_thumb->RenderToFile($this->cache_filename));
//                if (!$this->php_thumb->RenderToFile($this->cache_filename)) {
//                    trigger_error(sprintf(__d('thumb helper', 'Could not render image to: %s', true), $this->cache_filename), E_USER_WARNING);
//                }
        } else {
            $this->error = 1;
