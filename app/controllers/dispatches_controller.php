...
//            debug($this->params);
        ...
        /**
         * Return of input form fragment via Ajax to allow data entry for a new image
         */
        function new_image_record(){
            if(!isset($this->data)){
                $this->layout = '';
                $this->set('path', WWW_ROOT.'img'.DS.'uploads'.DS.'dispatches'.DS);
                $this->data = $this->Dispatch->find('all', array('conditions'=>array('img_file'=> $this->params['pass'][0])));
                $this->render('new_image_record', 'ajax');
            } else {
                $fields = array_keys($this->data);
                $img_file = array_keys($this->data['img_file']);
                $i =0;
                for($i; $i < count($this->data['id']); $i++) {
                    foreach($fields as $field){
                        if ($field === 'img_file'){
                            ...
                            foreach($img_file as $sub_key){
                                $data['Dispatch'][$i]['img_file'][$sub_key] = $this->data[$field][$sub_key][$i];
                            }
                        } else {
...
//                    $this->Dispatch->create();
                    $this->Dispatch->saveAll($data['Dispatch']);
